...
class Script_model extends CI_Model {
    ...
    public function get_all_scripts() {
        $this->trigger_events('pre_get_all_scripts');
        ...
        $this->response = $this->db->select('*')
                ...
                ->get();
        $this->trigger_events('post_get_all_scripts');
        return $this;
    }
    
    public function is_script_name_exists($name)
    {
        if (empty($name))
        {
            return FALSE;
    ...
    public function create_script($additional_data = array()) {
        $this->trigger_events('pre_create_script');
        //inserting into project_info table
        $this->db->insert($this->tables['project_info'], $additional_data);
        $id = $this->db->insert_id();
        if ( isset($id) ) {
            $this->set_message('Script is created successfully.');
        } 
        else
        {            
            $this->set_error('Error while creating a script.');
        }
        ...
        $this->trigger_events('post_create_script');
        return (isset($id)) ? $id : FALSE;
    }
